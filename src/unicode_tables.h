...
// Codepoint ranges for the Unicode categories the tokenizer's
// pre-tokenizer regex needs. Derived from Python's unicodedata so
// they cannot drift from what the reference tokenizer sees.
#include <stdint.h>

// 659 ranges
static const uint32_t kLetter[][2] = {
...
static const int kNLetter = 659;

// 137 ranges
static const uint32_t kNumber[][2] = {
...
static const int kNNumber = 137;

// 310 ranges
static const uint32_t kMark[][2] = {
    ...
    {0x898,0x89F}, {0x8CA,0x8E1}, {0x8E3,0x903}, {0x93A,0x93C}, {0x93E,0x94F}, {0x951,0x957},
...
static const int kNMark = 310;

static inline bool in_ranges(const uint32_t (*r)[2], int n, uint32_t c) {
  int lo = 0, hi = n - 1;
  while (lo <= hi) {
...
static inline bool uc_is_mark(uint32_t c)   { return in_ranges(kMark,   kNMark,   c); }
